...
# Inputs
air_pressure = int(sys.argv[1])  # QNH in HPA
airport_elevation = int(sys.argv[2])  # in feet
outside_air_temp = int(sys.argv[3])  # in C
# TREF = 44 # See document for description (in airport chart)
runway_length_uncorrected = int(sys.argv[4])
head_wind = int(sys.argv[5])
slope_percent = float(sys.argv[6])  # Percentage (uphill is positive, downhill is negative)
temperature = int(sys.argv[7])
aircraft_weight = float(sys.argv[8])
AP_registration = bool(sys.argv[9])  # Registration is AP-BLY or AP-BLZ
air_conditioning = bool(sys.argv[10])
engine_anti_ice = bool(sys.argv[11])
total_anti_ice = bool(sys.argv[12])
operational_CG_percentage = float(sys.argv[13])
...
# ---- CONFIG 1 ----
config1_0000_sheet = pd.ExcelFile(pre + '/10000.xlsx').parse()
config1_1000_sheet = pd.ExcelFile(pre + '/11000.xlsx').parse()
config1_2000_sheet = pd.ExcelFile(pre + '/12000.xlsx').parse()
...
# ---- CONFIG 2 ----
config2_0000_sheet = pd.ExcelFile(pre + '/20000.xlsx').parse()
config2_1000_sheet = pd.ExcelFile(pre + '/21000.xlsx').parse()
config2_2000_sheet = pd.ExcelFile(pre + '/22000.xlsx').parse()
# Prepare Data --> pressure altitude 0
...
# ---- CONFIG 3 ----
config3_0000_sheet = pd.ExcelFile(pre + '/30000.xlsx').parse()
config3_1000_sheet = pd.ExcelFile(pre + '/31000.xlsx').parse()
config3_2000_sheet = pd.ExcelFile(pre + '/32000.xlsx').parse()
# Prepare Data --> pressure altitude 0
